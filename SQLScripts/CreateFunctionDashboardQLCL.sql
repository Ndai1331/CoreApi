...
(
    SELECT
        ...
        WHERE
            ...
            AND (@Ward IS NULL OR qlcl.ward IN (SELECT TRIM(value) FROM STRING_SPLIT(@Ward, ',')) OR qlcl.ward IN (SELECT TRIM(value) FROM STRING_SPLIT(@Ward, ',')))
            AND qlcl.deleted = 0
        ...
        WHERE
            ...
            AND (@Ward IS NULL OR co_so_che_bien_nlts.ward IN (SELECT TRIM(value) FROM STRING_SPLIT(@Ward, ',')) OR co_so_nlts_du_dieu_kien_attp.ward IN (SELECT TRIM(value) FROM STRING_SPLIT(@Ward, ',')))
        ...
        FROM (
            ...
            WHERE
                ...
                AND (@Ward IS NULL OR kthk.ward IN (SELECT TRIM(value) FROM STRING_SPLIT(@Ward, ',')))
                AND kthk.deleted = 0
            GROUP BY 
